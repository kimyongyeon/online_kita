...
function deleteFile(newsNo, type)
{
	if(confirm('삭제 후에는 복원이 불가합니다. 그대로 삭제하시겠습니까?'))
	{
		$.ajax({
			...
			type: 'post',
			dataType: 'text',
			data: { 'newsNo' : newsNo, 'newsType' : 'WN', 'type': type},
			timeout: 30000,
			...
			<div class="contentsDetail">
				...
				<div class="location">
					...
					<span>
						<em>Main</em>&#62;
						<em>게시판 관리</em>&#62;
						<em class="on">Korean Wave 뉴스 등록 및 수정</em>
					</span>
				</div>
				<!-- //Location-->
				<h4 class="title">Korean Wav 등록 및 수정</h4>
				<form name="dataForm" action="koreanWaveNewsEditSubmit.jsp" enctype="multipart/form-data" method="post" >
				<input type="hidden" name="action" value="<%=action%>"/>
				<input type="hidden" name="newsNo" value="<%=newsNo%>"/>
				<input type="hidden" name="newsType" value="WN"/>
							...
							<tr>
								...
								<td colspan="5" style="text-align:left;padding-left:6px;">
									...
									<div class="orgFileName" style="margin-top:7px;">
										<a href="javascript:fileDownload('<%=orgListImage%>', '/boards/<%=listImage%>');"><%=orgListImage%></a>
				...
				<div class="buttonC">
					...
					<span class="button btnColor1 buttonSize2"><a href="koreanWaveNewsList.jsp">목 록</a><em></em></span>
				</div>
				</form>
			</div>
		</div>

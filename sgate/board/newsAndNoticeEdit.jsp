...
function deleteFile(newsNo, type)
{
	if(confirm('삭제 후에는 복원이 불가합니다. 그대로 삭제하시겠습니까?'))
	{
		$.ajax({
			...
			type: 'post',
			dataType: 'text',
			data: { 'newsNo' : newsNo, 'newsType' : 'NN', 'type': type},
			timeout: 30000,
			success: function (data) {
				if("SUCCESS" == $.trim(data))
				{
					alert("삭제되었습니다");
					if(type == 'IMAGE')
						$('input[name=orgListImage]').siblings('.orgFileName').css('display', 'none');
					else if(type == 'FILE')
						$('input[name=orgAttFile]').siblings('.orgFileName').css('display', 'none');
				}
				else
...
{
	if(confirm('삭제 후에는 복원이 불가합니다. 그대로 삭제하시겠습니까?'))
		location.href = "deleteNewsBoard.jsp?action=DELETE&newsType=NN&newsNo=" + newsNo;
}
</script>
</head>
<body>
			...
			<div class="contentsDetail">
				...
				<div class="location">
					...
					<span>
						<em>Main</em>&#62;
						<em>게시판 관리</em>&#62;
						<em class="on">News &#38; Notice 등록 및 수정</em>
					</span>
				</div>
				<!-- //Location-->
				<h4 class="title">News &#38; Notice 등록 및 수정</h4>
				<form name="dataForm" action="newsAndNoticeEditSubmit.jsp" enctype="multipart/form-data" method="post" >
				<input type="hidden" name="action" value="<%=action%>"/>
				<input type="hidden" name="newsNo" value="<%=newsNo%>"/>
				<input type="hidden" name="newsType" value="NN"/>
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="5" style="text-align:left;padding-left:6px;">
									<input type="file" name="orgListImage" class="fileUp" size="25"/>
									<label>*크기: 150*120</label>
									<%
									if(!"".equals(listImage) && !"".equals(orgListImage))
									{
									%>
									<div class="orgFileName" style="margin-top:7px;">
										<a href="javascript:fileDownload('<%=orgListImage%>', '/boards/<%=listImage%>');" style="text-decoration: underline;"><%=orgListImage%></a>
										<span> <a href="javascript:deleteFile(<%=newsNo%>, 'IMAGE');"> 삭제 </a> </span>
										<input type="hidden" class="changeFlag" name="imageChangeFlag" value="N"/>
									</div>
									<%
									}
									%>
							...
							<tr>
								...
								<td colspan="5" style="text-align:left;padding-left:6px;">
									...
									%>
									<div class="orgFileName" style="margin-top:7px;">
										<a href="javascript:fileDownload('<%=orgAttFile%>', '/files/<%=attFile%>');" style="text-decoration: underline;"><%=orgAttFile%></a>
				...
				<div class="buttonC">
					...
					<span class="button btnColor1 buttonSize2"><a href="newsAndNoticeList.jsp">목 록</a><em></em></span>
				</div>
				</form>
			</div>
		</div>

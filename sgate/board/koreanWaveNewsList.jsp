...
searchMap.put("newsType", "WN"); //Korean Wave News
...
function deleteBtn(newsNo)
{
	if(confirm('삭제 후에는 복원이 불가합니다. 그대로 삭제하시겠습니까?'))
		location.href = "deleteNewsBoard.jsp?action=DELETE&newsType=WN&newsNo=" + newsNo + '&pageNum=<%=pageNum%>';
}
</script>
</head>
<body>
			...
			<div class="contentsDetail">
				...
				<div class="location">
					...
					<span>
						<em>Main</em>&#62;
						<em>게시판 관리</em>&#62;
						<em class="on">Korean Wave 뉴스 관리</em>
					</span>
				</div>
				<!-- //Location-->
				<h4 class="title">Korean Wave 뉴스 관리</h4>
				<div class="searchBox">
					<form name="searchForm" action="koreanWaveNewsList.jsp" method="post">
				...
				<div class="buttonR">
					<span class="button btnColor1 buttonSize2"><a href="koreanWaveNewsEdit.jsp?action=INSERT">신규 등록</a><em></em></span>
				</div>
				<div class="tableType1">
					<table summary="뉴스 및 공지사항 게시판">
						<caption>뉴르및공지사항</caption>
						<colgroup>
							<col width="7%"/>
							<col width="*"/>
							<col width="13%"/>
						...
						else
						{
							...
							<tr>
								...
								<td class="left"><a href="koreanWaveNewsEdit.jsp?action=UPDATE&newsNo=<%=vnbListDao.getInt(i, "NEWS_NO")%>"><%=vnbListDao.getString(i, "TITLE")%></a></td>
								<td><%=util.getDate(vnbListDao.getString(i, "INPUT_DT"), "YYYY-MM-DD")%></td>
								<td><%=vnbListDao.getString(i, "READ_CNT")%></td>
								<td>
									<span class="button btnColor2 buttonSize1"><a href="koreanWaveNewsEdit.jsp?action=UPDATE&newsNo=<%=vnbListDao.getInt(i, "NEWS_NO")%>">수정</a><em></em></span>

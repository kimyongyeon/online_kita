...
String uploadImagePath = uploadPath + "images/";
...
String imageChangeFlag = util.nullCheck(multi.getRealFileName("imageChangeFlag")); //이미지 변경 상태
...
{
	if("UPDATE".equals(action) && "Y".equals(imageChangeFlag))
		vnbAction.deleteFile(newsNo, newsType, "IMAGE");

	//이미지는 이미지 폴더로 복사 후 삭제
	if(!"".equals(listImage))
	{
		util.copyFile(uploadTempPath, uploadImagePath, listImage);
		util.deleteFile(uploadTempPath, listImage);
	}
...
if(!"".equals(newsNo))
{
	if("INSERT".equals(action))
	{
		...
		nextUrl = "koreanWaveNewsEdit.jsp?action=UPDATE&newsNo=" + newsNo;
	}
	...
	else if("DELETE".equals(action))
	{
		...
		nextUrl = "koreanWaveNewsList.jsp";
	}
	else
	{
